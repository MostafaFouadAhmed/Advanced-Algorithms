...
import static topologicalorder.main.Vertex.createVertexWithChar;
...
public class Graph {
    ...
    public List<Vertex> createGraphUsingAdjacencyList(final int numberOfVertices, final List<Vertex> graph) {
        for (int i = 0; i < numberOfVertices; i++) {
            ...
            graph.add(createVertexWithChar(numberToAlphabet(i+1)));
        }
    ...
    public void setAdjacency(List<Pair<Vertex, Vertex>> pairList) {
        ...
        pairList.forEach(pair -> pair.getLeft().addAdjacentVertex(pair.getRight()));

    }

    public void setAdjacency(Pair<Vertex, Vertex> pair) {

       pair.getLeft().addAdjacentVertex(pair.getRight());
